...
import com.rs.game.npc.impl.boss.corp.CorporealBeast;
...
public class CorporealBeastCombat extends CombatScript {
    ...
    public int attack(final Npc npc, final Entity target) {
        ...
        if (attackStyle == 2) { // powerfull mage spiky ball
            ...
            delayHit(npc, 1, target, getMagicHit(npc, getRandomMaxHit(npc, 650, NpcCombatDefinitions.MAGIC, target)));
            World.sendProjectile(npc, target, 1825, 41, 16, 41, 0, 16, 0);
        } else if (attackStyle == 3) { // translucent ball of energy
            npc.setNextAnimation(new Animation(10410));
            delayHit(npc, 1, target, getMagicHit(npc, getRandomMaxHit(npc, 550, NpcCombatDefinitions.MAGIC, target)));
            if (target instanceof Player) {
                WorldTasksManager.schedule(new WorldTask() {
                    ...
                    public void run() {
                        ...
                        if (skill == Skills.PRAYER) player.getPrayer().drainPrayer(10 + Utils.getRandom(40));
                        else {
                            int lvl = player.getSkills().getLevel(skill);
                            lvl -= 1 + Utils.getRandom(4);
                            player.getSkills().set(skill, Math.max(lvl, 0));
                        }
                        player.getPackets()
            ...
            WorldTasksManager.schedule(new WorldTask() {
                ...
                public void run() {
                    for (int i = 0; i < 6; i++) {
                        ...
                        if (!World.canMoveNPC(newTile.getPlane(), newTile.getX(), newTile.getY(), 1)) continue;
                        World.sendProjectile(npc, tile, newTile, 1824, 0, 0, 25, 0, 30, 0);
                        for (Entity t : possibleTargets) {
                            if (Utils.getDistance(newTile.getX(), newTile.getY(), t.getX(), t.getY()) > 1 || !t
                                    .clippedProjectile(newTile, false)) continue;
                            delayHit(npc, 0, t,
                                    getMagicHit(npc, getRandomMaxHit(npc, 350, NpcCombatDefinitions.MAGIC, t)));
                        }
                        WorldTasksManager.schedule(new WorldTask() {
                            @Override
                            public void run() {
