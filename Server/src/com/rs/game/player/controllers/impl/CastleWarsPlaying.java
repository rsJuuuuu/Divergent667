...
public class CastleWarsPlaying extends Controller {
	...
	public void sendInterfaces() {
		player.getInterfaceManager().sendTab(
				player.getInterfaceManager().hasResizableScreen() ? 10 : 8, 58);
	}
	...
	public boolean processObjectClick1(WorldObject object) {
		 ...
		 * else if (id == 4438) player.getActionManager().setAction(new
			...
			for (List<Player> players : CastleWars.getPlaying()) {
				for (Player player : players) {
					if (player.withinDistance(object, 1))
						player.applyHit(new Hit(player, player.getHealth(),
