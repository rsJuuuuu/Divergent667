...
public class Smithing extends Action {
    ...
    public enum ForgingBar {
        ...
        ADAMANT(2361, 70, new Item[]{new Item(1211, 1), new Item(1357, 1), new Item(1430, 1), new Item(1145, 1),
                ...
                new Item(1271, 1)}, new double[]{62.5, 125, 187.5, 312.5}, new int[]{66, 210, 267}),

        BRONZE(2349, 0,
                new Item[]{new Item(1205, 1), new Item(1351, 1), new Item(1422, 1), new Item(1139, 1), new Item(877, 1),
                        new Item(1277, 1), new Item(819, 1), new Item(4819, 1), new Item(1794, 1), new Item(-1, 1),
                        new Item(-1, 1), new Item(39, 1), new Item(1321, 1), new Item(9420, 1), new Item(1291, 1),
                        new Item(864, 1), new Item(1155, 1), new Item(1173, 1), new Item(-1, 1), new Item(-1, 1),
                        new Item(1337, 1), new Item(1375, 1), new Item(1103, 1), new Item(1189, 1), new Item(3095, 1),
                        new Item(1307, 1), new Item(1087, 1), new Item(1075, 1), new Item(1117, 1), new Item(1265, 1)},
                new double[]{12.5, 25, 37.5, 62.5}, new int[]{66, 82, 210, 267}),

        IRON(2351, 15, new Item[]{new Item(1203, 1), new Item(1349, 1), new Item(1420, 1), new Item(1137, 1),
                ...
                new Item(1267, 1)}, new double[]{25, 50, 75, 125}, new int[]{66, 90, 162, 210, 267}),

        MITHRIL(2359, 50, new Item[]{new Item(1209, 1), new Item(1355, 1), new Item(1428, 1), new Item(1143, 1),
                ...
                new Item(1273, 1)}, new double[]{50, 100, 150, 250}, new int[]{66, 170, 210, 267}),

        RUNE(2363, 85, new Item[]{new Item(1213, 1), new Item(1359, 1), new Item(1432, 1), new Item(1147, 1),
                ...
                new Item(1275, 1)}, new double[]{75, 150, 225, 375}, new int[]{66, 210, 267}),

        STEEL(2353, 30, new Item[]{new Item(1207, 1), new Item(1353, 1), new Item(1424, 1), new Item(1141, 1),
    ...
    public boolean process(Player player) {
        if (!player.getInventory().containsItem(HAMMER, 1)) {
            player.getDialogueManager().startDialogue("SimpleMessage",
                    "You need a hammer in order to work with a bar of " + new Item(bar.getBarId(), 1).getDefinitions()
                            .getName().toLowerCase().replace(" bar", "") + ".");
            return false;
        }
        if (!player.getInventory().containsItem(bar.getBarId(), ForgingInterface
                .getActualAmount(bar.getLevel() + ForgingInterface.getFixedAmount(bar, bar.getItems()[index]), bar,
        ...
        if (player.getSkills().getLevel(Skills.SMITHING) < ForgingInterface.getLevels(index, player)) {
            player.getDialogueManager().startDialogue("SimpleMessage",
                    "You need a Smithing level of " + ForgingInterface.getLevels(index, player) + " to create this.");
            return false;
        }
    ...
    public int processWithDelay(Player player) {
        ...
        player.getInventory().deleteItem(bar.getBarId(), ForgingInterface
                .getActualAmount(bar.getLevel() + ForgingInterface.getFixedAmount(bar, bar.getItems()[index]), bar,
                        bar.getItems()[index].getId()));
    ...
    public static class ForgingInterface {
        ...
        public static void handleIComponents(Player player, int componentId) {
            ...
            for (int i = 3; i <= 6; i++) {
                for (int index = 0; index < componentChilds.length; index++) {
                    if (componentChilds[index] + i != componentId) continue;
                    slot = index;
        ...
        private static int getBasedAmount(Item item) {
            ...
            if (def.contains("dagger")) {
                ...
                return 3;
            } else if (def.contains("sword") || def.contains("dart tip") || def.contains("nails") || def
                    .contains("wire")) {
                return 4;
        ...
        public static String[] getStrings(Player player, ForgingBar bar, int index, int itemId) {
            ...
            levelString.append(TextUtils.formatPlayerNameForDisplay(name.replace(barVariableName + " ", "")));
        ...
        public static int getActualAmount(int levelRequired, ForgingBar bar, int id) {
            ...
            if (level >= 0 && level <= 4) {
                ...
                if (name.contains("knife") || name.contains("arrowtips") || name.contains("limb") || name
                        .contains("studs")) {
                    return 1;
                }
                return 2;
        ...
        public static void sendSmithingInterface(Player player) {
            ...
            for (int i = 0; i < bar.getItems().length; i++) {
                player.getPackets()
                        .sendItemOnIComponent(SMITHING_INTERFACE, componentChilds[i], bar.getItems()[i].getId(), 1);
            ...
            player.getPackets()
                    .sendIComponentText(300, 14, TextUtils.formatPlayerNameForDisplay(bar.toString().toLowerCase()) + "");
