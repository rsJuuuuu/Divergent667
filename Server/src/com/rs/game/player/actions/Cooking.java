...
public class Cooking extends Action {
    ...
    public enum Cookables {
         ...
         * Shark
         */
        RAW_SHARK(new Item(383, 1), 80, 200, 210, new Item(387, 1), new Item(385, 1), false, false),
         ...
         * Cavefish
         */
        RAW_CAVEFISH(new Item(15264, 1), 88, 200, 214, new Item(15268, 1), new Item(15266, 1), false, false),
         ...
         * RockTail
         */
        RAW_ROCKTAIL(new Item(15270, 1), 92, 200, 225, new Item(15274, 1), new Item(15272, 1), false, false),
    ...
    public boolean start(Player player) {
        ...
        if (cook.isFireOnly() && object.getId() != 2732) {
            ...
            player.getDialogueManager().startDialogue("SimpleMessage", "You need a cooking level of " + cook.getLvl()
                    + " to cook this food.");
            return false;
        }
        player.getPackets().sendGameMessage("You attempt to cook the " + cook.getProduct().getDefinitions().getName()
                .toLowerCase() + ".");
    ...
    private boolean isBurned(Player player) {
        int levelsToStopBurn = cook.getBurningLvl() - player.getSkills().getLevel(Skills.COOKING);
    ...
    public boolean process(Player player) {
        ...
        if (player.getSkills().getLevel(Skills.COOKING) < cook.getLvl()) {
            player.getDialogueManager().startDialogue("SimpleMessage", "You need a level of " + cook.getLvl() + " to " +
                    "" + "cook this.");
            return false;
        }
        return true;
    ...
    public int processWithDelay(Player player) {
        ...
        if (player.getSkills().getLevel(Skills.COOKING) < cook.getBurningLvl() && isBurned(player) && !(player
                .getEquipment().getGlovesId() == 775 && cook.getLvl() > 30 && player.getSkills().getLevel(Skills
                .COOKING) < cook.getBurningLvl() - (cook.getLvl() - 30))) {

            player.getInventory().deleteItem(item.getId(), 1);
            player.getInventory().addItem(cook.getBurntId().getId(), cook.getBurntId().getAmount());
            player.getPackets().sendGameMessage("Oops! You accidently burnt the " + cook.getProduct().getDefinitions
            ...
            player.getPackets().sendGameMessage("You successfully cook the " + cook.getProduct().getDefinitions()
                    .getName().toLowerCase() + ".", true);
        }
        if (amount > 0) {
            player.getPackets().sendGameMessage("You attempt to cook the " + cook.getProduct().getDefinitions()
                    .getName().toLowerCase() + ".");
            return 1;
        }
        return -1;
