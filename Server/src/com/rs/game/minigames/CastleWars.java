...
import com.rs.game.player.controllers.impl.CastleWarsPlaying;
...
public final class CastleWars {
	...
	public static void leavePlayersSafely(final int winner) {
		for (List<Player> aPlaying : playing) {
			for (final Player player : aPlaying) {
	...
	public static void refreshAllPlayersTime() {
		for (List<Player> aWaiting : waiting)
			for (Player player : aWaiting)
				refreshTimeLeft(player);
		for (int i = 0; i < playing.length; i++)
			for (Player player : playing[i]) {
	...
	public static boolean isBarricadeAt(WorldTile tile) {
        return playingGame != null && playingGame.isBarricadeAt(tile);
    }
